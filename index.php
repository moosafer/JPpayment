<?php
/*
 * Plugin Name: Pasargad Bank WooCommerce Gateway
 ...
 * Description: This is a payment gateway for adding Pasargad bank (an Iranian bank) gateway to your woocommerce site.
 * Version: 2.5
 * Author: AlaFalaki
...
function pasargad_bwg() {
    ...
    class pasargad_bwg_full_payment extends WC_Payment_Gateway {
		...
		//نمایش لوگوی بانک کنار اسم درگاه
		...
		function pasargad_bwg_check_response(){
			...
			if(($_SESSION['pasargadAmount']) == $order_total){
				...
				if($result['resultObj']['result'] == "True"){ // Check the result.
	                ...
	                $this -> msg['pasargad_bwg_class'] = 'woocommerce_error';
			        $this -> msg['pasargad_bwg_message'] = "پرداخت ناموفق.";
			...
			$redirect_url = add_query_arg( array('pasargad_bwg_message'=> urlencode($this->msg['pasargad_bwg_message']), 'pasargad_bwg_class'=>$this->msg['pasargad_bwg_class']), $redirect_url );
...
if( isset($_GET['pasargad_bwg_message']) )
{
	...
	{
		return '<div class="'.htmlentities($_GET['pasargad_bwg_class']).'">'.urldecode($_GET['pasargad_bwg_message']).'</div>'.$content;
	}
}
